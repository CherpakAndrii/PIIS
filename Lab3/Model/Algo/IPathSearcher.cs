...
public abstract class PathSearcher
{
    ...
    protected FieldModel Field { get; set; }
    private AdjacencyType Adjacency { get; }
    public abstract bool FindPath();
    
    public int FindPathLength(FieldModel fieldModel)
    {
        Field = fieldModel;
        if (!FindPath()) return -1;
        return Field[Field.EndPoint.Item1, Field.EndPoint.Item2].MinDistanceFromStart;
    }
    
    public RouteModel? FindPath(FieldModel fieldModel)
    {
        Field = fieldModel;
